...
class TestDmsStats:
    ...
    def test_stats_gibt_string_zurueck(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        (tmp_path / "data" / "dms").mkdir(parents=True)
        result = dms_stats()
        assert isinstance(result, str)

    def test_stats_enthaelt_symbol(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        (tmp_path / "data" / "dms").mkdir(parents=True)
        result = dms_stats()
        # Enthält Statistik-Symbole oder Fehlermeldung
        assert any(sym in result for sym in ["📊", "📁", "❌", "Dateien", "DMS"])
